classdef analyzer_multi_group_exported_1_4b < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function UpdateFreqs(app,row)
            ...
            freqRange = str2num(strrep(freqRangeU,"-"," ")); % str2num because multiple numbers are entered in the field to indicate a range
    ...
    methods (Access = private)
        ...
        function AnalyzeButtonPushed(app, event)
            ...
            for j=1:app.NumFiles
                ...
                if strcmpi(app.AnalysisMode, "discrete")
                    ...
                    thresh = max(abs(waveIntro.VFilt))*ambThresh;
                    ...
                    for i = 1:nSignals
                        ...
                        if ~app.SkipAmbientCheckBox.Value
                            % skip this check if we don't care about the ambient noise
                            if signalOnset < 1*Fs
                                % plot(app.WaveAxes,waveIn.Time,waveIn.VFilt);
                                % yline(thresh*1000);
                                error('Error: Signal onset less than 1 second from waveform start, cannot get 1 sec of ambient noise (%0.2f s)', signalOnset/Fs)
                            end
                        end
                        signalOffset = tempSignalTable.Offset(i+1);
                        if signalOffset > height(waveIn)
        ...
        function InputDiscreteTableCellEdit(app, event)
            ...
            if islogical(app.CoreTable.(colName)(row))
                newData = logical(str2num(newData));
            end
        ...
        function ModeButtonGroupSelectionChanged(app, event)
            ...
            if strcmpi(selectedButton, "Discrete")
                ...
                newColNames = {'Onset|(s)'; 'RMS|(dB re 1µPa)'; 'Peak|(dB re 1µPa)'; 'SNR'; 'SNR|(diff)'; 'CSEL'};
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.ResultsGrid.Layout.Column = 1;
            ...
            app.FileListBoxLabel.Enable = 'off';
            ...
            app.OutputGrid.Layout.Column = 1;
            ...
            app.InputGrid.ColumnWidth = {'1x', 'fit', '1x', 'fit'};
            ...
            app.DiscreteButton.Position = [49 1 66 22];
            ...
            app.AmbientButton.Position = [153 1 66 22];
            ...
            app.VersionLabel.Text = 'Version 1.4';
            ...
            app.v13NotesAddedbuttontoexportstatsforallanalyzedfilesLabel = uilabel(app.UIFigure);
            app.v13NotesAddedbuttontoexportstatsforallanalyzedfilesLabel.Position = [775 -102 342 59];
    ...
    methods (Access = public)
        ...
        % Construct app
        function app = analyzer_multi_group_exported_1_4b
